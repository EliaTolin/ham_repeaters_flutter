...
class ExamModeAccuracyUtils {
  static Map<ExamType, double> calculateExamTypeAccuracy(
      List<QuizSetQuestionResult> answers) {
    ...
    for (final examType in examTypes) {
      final answersForMode =
          answers.where((answer) => answer.exam == examType).toList();
      final accuracy =
          answersForMode.where((answer) => answer.isCorrect).length /
              answersForMode.length;
