...
import 'package:ham_repeaters/src/features/quiz/presentation/quiz_page/state/quiz_state/quiz_state.dart';
import 'package:ham_repeaters/src/features/quiz/provider/delete_quiz_set/delete_quiz_set_provider.dart';
import 'package:ham_repeaters/src/features/quiz/provider/get_custom_quiz_set/get_custom_quiz_set_provider.dart';
import 'package:ham_repeaters/src/features/quiz/provider/get_quiz_from_curated_set/get_quiz_from_curated_set_provider.dart';
import 'package:ham_repeaters/src/features/quiz/provider/get_quiz_set/get_quiz_set_provider.dart';
import 'package:ham_repeaters/src/features/quiz/provider/get_topics/get_topics_provider.dart';
import 'package:ham_repeaters/src/features/quiz/provider/set_quiz_finished/set_quiz_finished_provider.dart';
...
class QuizController extends _$QuizController {
  ...
  FutureOr<QuizState?> build({
    ...
    if (examType != null) {
      ...
      state = await AsyncValue.guard(() async {
        final quizSet =
            await ref.read(getQuizFromCuratedSetProvider(curatedSetId).future);
        final allTopics = await ref.read(getTopicsProvider.future);
        return QuizState(
          quizSet: quizSet,
          currentQuestionIndex: 0,
  ...
  void goToQuestion(int index) {
    ...
    if (currentState == null ||
        index < 0 ||
        index >= currentState.totalQuestions) return;
  ...
  Future<void> submitQuiz() async {
    ...
    try {
      await ref
          .read(setQuizFinishedProvider(currentState.quizSet.quizSetId).future);
  ...
  Future<void> deleteQuiz() async {
    ...
    try {
      await ref
          .read(deleteQuizSetProvider(currentState.quizSet.quizSetId).future);
    } catch (e, st) {
      await Sentry.captureException(e, stackTrace: st);
    }
  }
